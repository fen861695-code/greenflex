...
内置 GreenConcierge 绿色推理管家，支持两种模式：

**智能管家模式**（默认）：用自然语言描述任务，管家自动分析并推荐模型
- 自动识别任务类型（分类、抽取、摘要、生成、分析、代码、翻译）
- 返回推荐模型、价格、能耗、碳排、耗时和备选方案
- 查询电网碳信号、模型列表、订单状态
- 配置云 API Key（推荐 DeepSeek）后启用 LLM function calling，可直接对话和执行任务
- 未配置 Key 时使用规则模式，仍可完成任务分析和推荐
