...
async def seed_catalog(session: AsyncSession) -> None:
    ...
    Automatically populates energy provenance metadata (v2).
    """
    ...
    for values in MODEL_SEEDS + CLOUD_MODEL_SEEDS:
        if values["id"] not in existing:
            enriched = _enrich_energy_provenance(values)
...
def _enrich_energy_provenance(values: dict) -> dict:
    ...
    # Benchmark coverage: which (family, param_size) pairs have L2 data
    # From JouleBench (A100 FP16) + arXiv 2608.00008 (4060Ti Q4) + Watt Counts (H100)
    l2_coverage = {
    ...
    # Families that have at least one benchmark (for L3 interpolation)
    l3_families = set(l2_coverage.keys())

    if "cloud" in model_id:
        # Cloud API models: no local benchmark data available
        provenance = "insufficient_data"
        confidence = 0
        source = f"Cloud API model: no verified inference energy benchmark data"
        reference_gpu = None
    elif param_count is not None and arch_family in l2_coverage:
        # Check if exact parameter count has L2 benchmark
        exact_match = any(
        ...
        # Family has benchmarks but param count unknown = L3
        provenance = "l3_cross_gpu_normalized"
        confidence = 6000
        source = f"Interpolated from {arch_family} family benchmarks"
        reference_gpu = "a100-80gb"
    else:
        # No benchmark coverage for this architecture = insufficient data
        provenance = "insufficient_data"
        confidence = 0
